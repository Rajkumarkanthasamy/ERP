...
import { PR_LIMIT, PR_LINE_STATUS, PR_STATUS, computePoAmounts } from '../constants.js';
...
export async function createPRs({ projectCode, productNo, lines, remarks, user }) {
  ...
  try {
    for (const pack of packs) {
      ...
      const vendorName = await getVendorName(pack.vendorCode);
      ...
      headerReq.input('VendorName', sql.NVarChar, vendorName);
      ...
      for (const line of pack.lines) {
        ...
        d.input('VendorName', sql.NVarChar, vendorName);
...
export async function approvePO(poRef, { step, remarks, user }) {
  const name = user.displayName || user.username;
  const lines = (
...
export async function createGRN({ poRef, vendorCode, projectCode, remarks, lines, user }) {
  ...
  try {
    ...
    hdr.input('PORef', sql.NVarChar, poRef || null);
    hdr.input('Vendor', sql.NVarChar, vendorCode || null);
    hdr.input('Project', sql.NVarChar, projectCode || null);
    ...
    await tx.commit();
    return { grnNumber, poRef, status: 'Received' };
  } catch (err) {
    await tx.rollback();
